...
function HomeView(props) {
    ...
    const initalSettings = () => {
      ...
      if (
        typeof localStorage.getItem(
          "activity-dbtdiarycard-" + props?.data?.activity?.id
        ) !== "undefined" &&
  ...
  if (active === 0) {
    ...
    return (
      <TargetView
        activityId={props?.data?.activity?.id ?? null}
        settings={settings}
        {...props}
    ...
    return (
      <SkillsView
        {...props}
        activityId={props?.data?.activity?.id ?? null}
        onContinue={(mode) => setActive(mode)}
        onBack={() => setActive(2)}
      />
    );
  } else if (active === 4) {
    ...
    return (
      <NoteView
        {...props}
        activityId={props?.data?.activity?.id ?? null}
        onContinue={() => setActive(8)}
        onBack={() =>
          setActive(
            !props.report || (props.report && props.report.skillToday) ? 6 : 42
          )
        }
      />
    );
