...
class Board extends React.Component<BoardProps, BoardState> {
    ...
    this.timer = setInterval(() => {
      if (this.state.showNo === 1) {
        ...
        setTimeout(() => {
          ...
          // Game over interval
          const timeoutPeriod = this.props.level === 1 ? 60000 : 80000;
          setTimeout(() => {
            this.setState({
              completed: true,
            });
  ...
  getBubbleStyles = (): Array<string> => {
    ...
    for (let i = 0; i < this.props.bubbleCount; i++) {
      ...
      this.count =
        toBeSelected.indexOf(this.classes[selectedStyle]) > -1
          ? this.count + 1
          : this.count;
    }
  ...
  handleClick = (
    ...
    const pointsToReduce =
      typeof this.state.bubbleStyles[index - 1] !== "undefined" &&
      lastClass !== this.state.bubbleStyles[index - 1]
        ? 1
        : 0;
    const success =
      this.props.level === 1
