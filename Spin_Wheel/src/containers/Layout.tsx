...
const Layout = ({ ...props }) => {
  ...
  const highRiskConditionsWinWheel = convertObjtoArray(
    settingsData?.high_risk[0]?.win,
    settingsData?.high_risk[0]?.zero?.probability
  );
  const lowRiskConditionsLoseWheel = convertObjtoArray(
    settingsData?.low_risk[0]?.loose,
    settingsData?.low_risk[0]?.zero?.probability
  );
  const lowRiskConditionsWinWheel = convertObjtoArray(
    settingsData?.low_risk[0]?.win,
    settingsData?.low_risk[0]?.zero?.probability
  );
  ...
  useEffect(() => {
    ...
    const settings =
      props.data.activity?.settings ?? props.data.settings ?? null;
  ...
  return (
    <div className="layout">
      <AppBar
        position="static"
        style={{ background: "rgba(53,159,254,1)", boxShadow: "none" }}
      >
