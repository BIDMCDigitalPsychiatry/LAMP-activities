...
class Board extends React.Component<BoardProps, DiamondState> {
  ...
  updateStateWithTaps = (i: number, statusVal: boolean) => {
    const routes = [];
    ...
    if (this.state.startTimer > 0 && lastClickedItems.indexOf(i) < 0) {
      const route = {
        ...
        value: null,
        status: statusVal,
        duration: statusVal && i === 1 ? 0 : lastclickTime,
        level: 1,
      };
      routes.push(route);
    }
    this.setState(
      {
        ...
        stepNumber: statusVal === true ? i : this.state.stepNumber,
      },
      () => {
        if (statusVal === true && this.props.totalDiamonds === i) {
          ...
          children.push(
            <td key={p}>
              <Diamond
                ...
                diamondNumber={this.props.diamondNumber}
